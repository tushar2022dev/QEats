...
public class RestaurantServiceMockitoTestStub {
  ...
  protected RestaurantRepositoryService restaurantRepositoryServiceMock;
    ...
    private Restaurant[] initializeRestaurantObjectsForOffPeak() throws IOException {
    String fixture =
  ...
  private Restaurant[] initializeRestaurantObjectsForPeak() throws IOException {
    String fixture =
  ...
  public void  testFindNearbyWithin5km() throws IOException {
    ...
    // Test Input: restaurantService.findAllRestaurantsCloseBy(should be inline with 
    // your mock setup)
    ...
    when(restaurantRepositoryServiceMock
            .findAllRestaurantsCloseBy(any(Double.class), any(Double.class),
                eq(LocalTime.of(3, 0)),
                any(Double.class)))
            .thenReturn(Arrays.asList(initializeRestaurantObjectsForOffPeak()));
    ...
    assertEquals(5, allRestaurantsCloseBy.getRestaurants().size());
    assertEquals("11", allRestaurantsCloseBy.getRestaurants().get(0).getRestaurantId());
    assertEquals("14", allRestaurantsCloseBy.getRestaurants().get(1).getRestaurantId());
  ...
  public void  testFindNearbyWithin3km() throws IOException {
    ...
    List<Restaurant> restaurantList1 = Arrays.asList(initializeRestaurantObjectsForOffPeak());
    List<Restaurant> restaurantList2 = Arrays.asList(initializeRestaurantObjectsForPeak());

    // TODO: CRIO_TASK_MODULE_MOCKITO
    //  Initialize these two lists above such that I will match with the assert statements
    //  defined below.

    doReturn(restaurantList1)
        .when(restaurantRepositoryServiceMock)
    ...
    assertEquals(5, allRestaurantsCloseByOffPeakHours.getRestaurants().size());
